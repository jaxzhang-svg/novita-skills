...
Complete implementation of the standalone agent core with hooks.
...
// Hook into events
agent.on('message:user', (msg) => console.log('User:', msg.content));
agent.on('message:assistant', (msg) => console.log('Assistant:', msg.content));
...
The agent uses EventEmitter3 for extensibility. All operations emit events:

### Message Events
- `message:user` - Emitted when user message is added
- `message:assistant` - Emitted when assistant response is complete

### Streaming Events
- `item:update` - Emitted for each streaming item (items-based)
- `stream:start` - Emitted when streaming begins
- `stream:delta` - Emitted for each text chunk
- `stream:end` - Emitted when streaming completes

### Tool Events
- `tool:call` - Emitted when tool is called
- `tool:result` - Emitted when tool returns result

### State Events
- `thinking:start` - Emitted when agent starts processing
- `thinking:end` - Emitted when agent finishes processing
